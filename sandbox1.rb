...
class LineAnalyzer
  ...
  def initialize(in_line, in_lineno)      # CONSTRUCTOR 
    ...
    #  which is internal to class - this will set the Instance Vars that hold the
    #  words and counts
  ...
  def calculate_word_frequency
    ...
    if ( in_content.empty? == true)
      ...
      # place those into "temparray" - then copy that to the Instance var @highest_wf_words ( array of words )
      temparray = []
...
class Solution
  ...
  def initialize  # CONSTRUCTOR 
    ...
    @highest_count_words_across_lines = 0
  end   
  ...
  def calculate_line_with_highest_frequency()
    ...
    if (@@dbg == true)
      puts " DBG: Result highest Count, Word(s) : #{analyzers[0].highest_wf_count} , #{analyzers[0].highest_wf_words} "
    end
...
# now parse the file specicied, counting the max words in each line
sol.analyze_file( filename )

puts "checking--- is it nil?"; puts sol.highest_count_across_lines.nil?
...
print "-- checking highest cnt : "; puts sol.highest_count_across_lines
